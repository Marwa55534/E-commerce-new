<div>
    ...
    <div class="product-availability">
        ...
        <span class="inner-text">
            ...
            {{-- <div>Debug: manage_stock = {{ var_dump($product->manage_stock) }}, quantity = {{
                var_dump($product->quantity) }}</div> --}}
            ...
            @if ($product->has_variants)
                {{$quantity}} in stock
            @else
                {{$product->manage_stock == 1 ? $product->quantity . 'in stock' : 'Available'}}
            @endif
        </span>
    </div>
        ...
        <div class="product-size">
            ...
            <ul class="size-option">
                @foreach ($variants as $variant)
                    <a wire:click="changeVariant({{$variant->id}})" href="javascript:void(0)" class="option">
                        @foreach ($variant->VariantAttributes as $variantAttribute)

                            {{-- @selected($variant->id == $variantId) --}}
                            <span class="option-text">{{$variantAttribute->attributeValue->attribute->name}}
...
// Dropdown toggle functionality
    document.addEventListener('click', function (event) {
        const toggleBtn = event.target.closest('.size-section');
        if (toggleBtn) {
            const sizeOption = toggleBtn.querySelector('.size-option');
            if (sizeOption) {
                sizeOption.classList.toggle('active');
            }
        } else {
            // Close dropdown if clicking outside
            document.querySelectorAll('.size-option.active').forEach((dropdown) => {
